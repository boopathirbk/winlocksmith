        ...
        <div className="py-16 px-4 max-w-3xl mx-auto">
            <div className="dark:bg-zinc-900/40 bg-white dark:border-zinc-800/40 border-zinc-200 border rounded-2xl p-8 md:p-10">
                ...
                <div className="flex flex-col md:flex-row items-center gap-6 mb-10">
                    <div className="w-28 h-28 rounded-2xl bg-gradient-to-br from-sky-500 to-blue-600 flex items-center justify-center shadow-lg shadow-sky-500/15 shrink-0">
                        <span className="text-3xl font-bold text-white tracking-tight">BR</span>
                    </div>
                    <div className="text-center md:text-left space-y-2.5">
                        ...
                        <p className="text-sm text-sky-500 dark:text-sky-400 font-medium">Senior IT Support Executive & Web Developer</p>
                        <div className="flex flex-wrap justify-center md:justify-start gap-3 text-xs dark:text-zinc-500 text-zinc-500">
                            <span className="flex items-center gap-1.5"><ICONS.MapPin className="w-3.5 h-3.5" /> Bengaluru, India</span>
                            <a href="mailto:boopathirbk77@gmail.com" className="flex items-center gap-1.5 dark:hover:text-white hover:text-zinc-900 transition-colors"><ICONS.Mail className="w-3.5 h-3.5" /> boopathirbk77@gmail.com</a>
                            <a href="tel:+917395845142" className="flex items-center gap-1.5 dark:hover:text-white hover:text-zinc-900 transition-colors"><ICONS.Phone className="w-3.5 h-3.5" /> +91 7395845142</a>
                        </div>
                ...
                <div className="space-y-8">
                    ...
                    <Section title="Experience">
                        <div className="space-y-5">
                            <ExperienceItem role="Senior IT Support Executive & Web Developer" company="RSJ INSPECTION SERVICE LTD." period="Apr 2024 – Present" location="Bengaluru">
                                <ul className="space-y-1 dark:text-zinc-500 text-zinc-500 text-[13px]">
                    ...
                    <Section title="Education">
                        <div>
                            ...
                            <p className="dark:text-zinc-500 text-zinc-500 text-[13px]">Bannari Amman Institute of Technology, Anna University · 2015–2019</p>
                        </div>
                    </Section>
                    ...
                    <Section title="Certifications">
                        <div className="grid sm:grid-cols-2 gap-2">
                            {['AWS Certified Cloud Practitioner', 'Microsoft Certified: Azure Fundamentals', 'Microsoft 365 Certified: Fundamentals', 'Linux Essentials: 010 (LPI)'].map(cert => (
                                <div key={cert} className="flex items-center gap-2 text-[13px] dark:text-zinc-400 text-zinc-600 p-2.5 dark:bg-zinc-800/30 bg-zinc-50 rounded-lg border dark:border-zinc-800/30 border-zinc-100">
...
const Section = ({ title, children }: { title: string, children: React.ReactNode }) => (
    <div className="space-y-3">
        <h3 className="text-sm font-semibold dark:text-white text-zinc-900 uppercase tracking-wider dark:text-zinc-300 border-b dark:border-zinc-800/40 border-zinc-200 pb-2">{title}</h3>
        {children}
    </div>
);
...
const ExperienceItem = ({ role, company, period, location, children }: any) => (
    <div className="space-y-1.5">
        <div className="flex flex-col sm:flex-row justify-between sm:items-baseline">
            ...
            <span className="dark:text-zinc-600 text-zinc-400 text-xs font-mono">{period}</span>
        </div>
        <p className="text-sky-500 dark:text-sky-400 text-xs">{company} · {location}</p>
...
const Badge = ({ children, color = 'sky' }: { children: string, color?: 'sky' | 'rose' | 'emerald' }) => {
    const colors = {
        sky: 'dark:bg-sky-500/8 bg-sky-50 dark:text-sky-400 text-sky-700 dark:border-sky-500/15 border-sky-200',
        rose: 'dark:bg-rose-500/8 bg-rose-50 dark:text-rose-400 text-rose-700 dark:border-rose-500/15 border-rose-200',
        emerald: 'dark:bg-emerald-500/8 bg-emerald-50 dark:text-emerald-400 text-emerald-700 dark:border-emerald-500/15 border-emerald-200',
    };
    return (
...
const SocialLink = ({ href, icon, label }: any) => (
    <a href={href} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 px-2.5 py-1 dark:bg-zinc-800 bg-zinc-100 dark:hover:bg-zinc-700 hover:bg-zinc-200 rounded-lg text-xs dark:text-zinc-300 text-zinc-700 transition-colors border dark:border-zinc-700/50 border-zinc-200">
        {icon} {label}
    </a>
);
